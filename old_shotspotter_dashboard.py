...
# create tabs ------
tab_executive_summary, tab_event_alert, tab_events, tab_casings, tab_injuries, tab_arrests = st.tabs(
    ["Executive Summary", "Events and Alerts","Events", "Shell Casings", "Injuries", "Arrests"]
)
...
with tab_event_alert:
    st.header("Shotspotter alerts vs. Reported Evidence")

    # user selection for events on this tab
    option_alert_event = st.multiselect(
        "Please select what makes up evidence (i.e. events for this tab only)",
    ...
    df_event_alert = df_filtered.fillna(
        {
            "casings": 0,
            "injuries": 0,
            "arrests": 0
    ...
    for bcol, metric, score in zip((bcol1, bcol2, bcol3, bcol4), metrics.keys(), metrics.values()):
        with bcol:
            st.metric(label=metric, value=round(score, 3))
