...
a1.metric("No. of Yr", f"{st.session_state["dict_variables_summary"]['count_years_event_counts']}")
a2.metric(
"Alert: Avg. Events Per Yr.",
f"{st.session_state["dict_variables_summary"]['mean_var_true_event_counts']:.2f}",
)
a3.metric(
"No alert: Avg. Events Per Yr.",
f"{st.session_state["dict_variables_summary"]['mean_var_false_event_counts']:.2f}",
)
b1.metric(
"Diff. in Avg. Events Per Yr.",
f"{st.session_state["dict_variables_summary"]['mean_diff_event_counts']:.2f}",
)
b2.metric(
"Percent Diff. Avg. Events Per Yr.",
f"{st.session_state["dict_variables_summary"]['mean_percentage_diff_event_counts']:.2%}",
)
...
st.subheader("Events By Year and Alert")
display_linechart_viz(y_col="event_counts", color_col="colour")

st.subheader("Locations of Alerts and Public Devices")
st.pydeck_chart(
    pdk.Deck(
